...
class UserGUI:
    ...
    def create_history_tab(self):
        ...
        ttk.Label(history_tab, text="View Purchase History", font=("Arial", 14)).grid(row=0, column=0, pady=10)
        ttk.Label(history_tab, text="Username:").grid(row=1, column=0, pady=5)

        self.history_username_entry = ttk.Entry(history_tab, width=30)
        self.history_username_entry.grid(row=1, column=1, pady=5)

        ttk.Button(history_tab, text="View History", command=self.view_purchase_history).grid(row=2, column=1, pady=10)

        self.history_display = tk.Text(history_tab, width=70, height=15)
    ...
    def view_purchase_history(self):
        """Displays purchase history for a user."""
        username = self.history_username_entry.get()
        user = next((u for u in self.system.users if u.username == username), None)  # Composition with User.
        if user:
            history = user.view_purchase_history()
            self.history_display.delete(1.0, tk.END)
            for record in history:
                self.history_display.insert(
                    tk.END, f"Ticket: {record['ticket_type']}, Quantity: {record['quantity']}, Total: {record['total_price']} DHS\n"
                )
        else:
